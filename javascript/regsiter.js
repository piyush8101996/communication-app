...
registerHandler.addEventListener('click',function(e){
    ...
    if (fullname.value.length > 0 && password.value.length > 0 && email.value.length >0 && password.value===cpassword.value && email.value.match(mailformat)) {
        ...
        const existarray=user_records.filter((user)=>user.email===email.value)
    ...
    //fullname validation
    if (fullname.value === "") {
        document.getElementById('fnameerror').innerHTML = "Fullname Feild Should not be empty"
        document.getElementById('fullname').classList.add('email')
    }else if(fullname.value.length<6){
        document.getElementById('fnameerror').innerHTML = "Fullname should be greater than 6"
        document.getElementById('fullname').classList.add('email')
    }else{
    ...
    console.log(email)
    if(email.value === ""){
        document.getElementById('emailerror').innerHTML = "Email feild should not be empty"
        document.getElementById('email').classList.add('email')
    } else if (!email.value.match(mailformat))  {
        document.getElementById('emailerror').innerHTML = "Email Format is not valid"
        document.getElementById('email').classList.add('email')
    }else{
    ...
    if (password.value === "") {
        ...
        document.getElementById('passerr').innerHTML ="password should be greater than 6"
        document.getElementById('password').classList.add('email')
    ...
    if(cpassword.value ===''){
        document.getElementById('cpasserr').innerHTML = "Confirm Password feild should not be empty"
        document.getElementById('cpassword').classList.add('email')
    }else if(cpassword.value !== password.value){
        document.getElementById('cpasserr').innerHTML ="password and confirm password doesnt match"
        document.getElementById('cpassword').classList.add('email')
    }else{
